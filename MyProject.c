// Configuration of I/O pins for PIC16F877A
...
#define ULTRASONIC_LED 0x10        // RA4 to indicate ultrasonic detection
...
interrupt() {
    while (!(PORTB & LINE_SENSOR) && !(PORTB & LINE_SENSOR2)) {
        ...
        lineSensorsTriggered = 1;  // Set the flag to indicate backward motion
        moveBackward();
    }
    lineSensorsTriggered = 0;  // Reset the flag when the lines are off
...
void main() {
    ...
    init();            // Initialize I/O, ADC, etc.
    initPWM();         // Initialize PWM settings
